    ...
    async function vizLoadNetwork() {
      ...
      try {
        ...
        if (cfg.location) {
          ...
          locRows = await fetchAllPages(cfg.base + cfg.location, logEl, fLocSrc, 'PRDID,LOCFR,LOCID,TLEADTIME');
        ...
        if (cfg.customer) {
          ...
          custRows = await fetchAllPages(cfg.base + cfg.customer, logEl, fCustSrc, 'PRDID,LOCID,CUSTID,CLEADTIME');
        ...
        if (cfg.sourceProd) {
          ...
          plantRows = await fetchAllPages(cfg.base + cfg.sourceProd, logEl, fPsh, 'SOURCEID,PRDID,LOCID,PLEADTIME');
